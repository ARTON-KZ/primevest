...
async function sendMail({ to, subject, html, text }) {
  const from = process.env.MAIL_FROM || `${BRAND} <no-reply@primevest.com>`;
      ...
      <tr><td align="center">
        <table role="presentation" width="520" cellpadding="0" cellspacing="0"
          ...
          <tr><td style="padding:28px 32px;border-bottom:1px solid #1d2c47;">
            <span style="font-size:20px;font-weight:700;color:#fff;letter-spacing:-0.02em;">Prime<span style="color:${GREEN};">Vest</span></span>
          </td></tr>
          <tr><td style="padding:36px 32px 8px;">
            ...
            <p style="margin:0 0 18px;color:#aab4c8;font-size:15px;line-height:1.7;">
              Your PrimeVest account is live. You're one step away from putting your
          ...
          <tr><td style="padding:24px 32px;border-top:1px solid #1d2c47;color:#5a6678;font-size:12px;">
            You're receiving this because you created a PrimeVest account.
          </td></tr>
        </table>
      </td></tr>
    </table>
  </div>`;
  const text = `Welcome aboard, ${first}!\n\nYour PrimeVest account is live. Make your first deposit to start growing your balance: ${depositUrl}\n\nDeposits are made to a secure crypto address shown in your dashboard.`;
  return { html, text };
}

async function sendWelcome(user) {
  const { html, text } = welcomeTemplate(user.name);
  return sendMail({ to: user.email, subject: 'Welcome to PrimeVest — let’s get you started', html, text });
}
